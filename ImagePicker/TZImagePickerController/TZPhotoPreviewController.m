...
@interface TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate,CLImageEditorDelegate, CLImageEditorTransitionDelegate, CLImageEditorThemeDelegate> {
...
- (void)configBottomToolBar {
    ...
    editButton.frame = CGRectMake(12, 0, 44, 44);
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake((CGRectGetWidth(_toolBar.frame) - fullImageWidth - 56) / 2 - 10, 0, fullImageWidth + 56, 44);
    ...
    [_toolBar addSubview:editButton];
...
- (void)editImage:(UIImage *)image {
    ...
    //theme.toolbarColor = [UIColor colorWithRed:(34/255.0) green:(34/255.0)  blue:(34/255.0) alpha:1.0];
...
- (void)imageEditor:(CLImageEditor *)editor didFinishEditingWithImage:(UIImage *)image {
    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (_tzImagePickerVc.imagePickerDidEditFinish) {
        _tzImagePickerVc.imagePickerDidEditFinish(image);
    }
    [self.navigationController dismissViewControllerAnimated:YES completion:nil];
}
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        CGRect rect = _tzImagePickerVc.cropRect;
        if (rect.size.width > cell.previewView.imageView.frame.size.width) {
            rect.origin.x += (rect.size.width - cell.previewView.imageView.frame.size.width) / 2;
            rect.size.width = cell.previewView.imageView.frame.size.width;
        }
        if (rect.size.height > cell.previewView.imageView.frame.size.height) {
            rect.origin.y += (rect.size.height - cell.previewView.imageView.frame.size.height) / 2;
            rect.size.height = cell.previewView.imageView.frame.size.height;
        }
        UIImage *cropedImage = [TZImageCropManager cropImageView:cell.previewView.imageView toRect:rect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
